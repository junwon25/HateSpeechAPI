...
#get model
model = ElectraForSequenceClassification.from_pretrained("KoElectra_base3")
model.load_state_dict(torch.load("model_10epoch.pt", map_location=torch.device('cpu')))
model_tokenizer = AutoTokenizer.from_pretrained("KoElectra_base3")
...
def result():
   if request.method == 'POST':
      ...
      return render_template("prediction.html",result = val) #name은 key, name에 저장된 값은 value
